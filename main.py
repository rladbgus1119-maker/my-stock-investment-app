...
st.set_page_config(page_title="AI 퀀트 유니버스: 토스 미니멀", layout="wide")
...
def draw_toss_chart(df, name, ticker, period):
    chart_df = df.copy()
    y_vals = chart_df['Close'] * (EXCHANGE_RATE if ".KS" not in ticker else 1)
    
    fig = go.Figure()
    
    # 💡 호버 툴팁 포맷 설정 (날짜/시간 + 가격)
    hover_fmt = "%Y-%m-%d %H:%M" if period in ["1일", "1주"] else "%Y-%m-%d"
    
    fig.add_trace(go.Scatter(
        x=chart_df.index, y=y_vals,
        mode='lines', line=dict(color='#3182f6', width=3),
        # 💡 마우스 갖다대면 시간, 날짜, 가격 뜨는 형식
    ...
    fig.update_layout(
        ...
        # 💡 X축 지저분한 날짜 라벨 제거 (1일 선택 시 더욱 깔끔하게)
        xaxis=dict(
            showgrid=False, 
            showticklabels=False if period == "1일" else True, # 1일일 때는 라벨 숨김
...
if not st.session_state.user_name:
    st.title("🏆 AI 투자 서바이벌: 시즌 1")
    with st.container(border=True):
        u_name = st.text_input("닉네임 입력")
...
total_stock_val = 0
for s in STOCK_MAP:
    p, _ = fetch_robust_data(s, period="1일") # 대시보드용은 항상 1일 실시간가
    live_prices[s] = p
...
page = st.sidebar.radio("🧭 메뉴", ["🏠 대시보드", "🛒 실시간 거래소", "📚 아카데미", "🛍️ 상점/커뮤니티"])
...
if page == "🏠 대시보드":
    ...
    st.divider()
    # 랭킹 차트 간략 보기
    st.subheader("🏆 현재 리그 순위")
    my_r = {"닉네임": f"{st.session_state.user_name} (나) ⭐", "자산": total_assets}
    ranks = sorted(st.session_state.bots + [my_r], key=lambda x: x['자산'], reverse=True)
    for idx, p in enumerate(ranks[:3]):
        st.write(f"{idx+1}위: {p['닉네임']} | {p['자산']:,.0f}원")

elif page == "🛒 실시간 거래소":
    m_col, r_col = st.columns([1.8, 1.2])
    with m_col:
        target = st.selectbox("종목 선택", list(STOCK_MAP.keys()))
        period_choice = st.radio("기간", ["1일", "1주", "3달", "1년", "5년", "전체"], horizontal=True, label_visibility="collapsed")
        st.session_state.selected_period = period_choice
        
        _, df_selected = fetch_robust_data(target, period=period_choice)
        if df_selected is not None:
            st.plotly_chart(draw_toss_chart(df_selected, target, STOCK_MAP[target], period_choice), use_container_width=True)
    ...
    with r_col:
        if not st.session_state.is_ended:
            st.markdown(f'<div class="timer-box">⏳ {int(sec_left//60)}분 {int(sec_left%60)}초 후 시즌 종료</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="timer-box">🏁 시즌 종료! TOP 3 승급 가능</div>', unsafe_allow_html=True)
